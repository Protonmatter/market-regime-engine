...
<!-- ci-status-start -->
- **Tests:** 1312 passing (`pytest -q -m "not slow"`) on the v1.6.1
  branch (post-v1.6.0); v1.5.1 PR-9 baseline was 1006, v1.5.0 baseline
  was 916. The 19 pre-existing env failures (slowapi/duckdb extras not
  installed) are unchanged.
- **Ruff:** `ruff check src tests` and `ruff format --check src tests`
  exit 0.
- **Mypy:** 0 errors (v1.6.0 Phase 5 brought the count to zero from
  the v1.5.1 PR-9 baseline of 13/14; v1.5.0 baseline was 34). The
  `python -m mypy src/market_regime_engine` quality bar is now
  zero-errors and must hold.
- **DuckDB E2E smoke:** sample → audit → score → `verify-run.approved`
  in **0.33s** wall-clock under DuckDB default
  (`tests/test_warehouse_duckdb_appender.py::test_warehouse_smoke_against_duckdb_under_60s`).
- **Audit zip:** **0.96 MB** (v1.4.0); ≤ 5 MB ceiling.
- **DuckDB bulk-write 10k rows:** **0.64s** (v1.3 was ≈ 427s).
- **PR-9 indexed-SQL fast path:** `latest_credit_regime_score` on a
  100k-row fixture now hits p99 ≤ 25 ms (CI envelope; bare-metal
  hits ≤ 5 ms per the PR-9 plan).
- Initial commit `741e51a`; v1.1 commit `79249df`; v1.2 commit
  `904d058`; v1.2.1 commit `52de50d`; v1.3 commit `ce73c14`; v1.4
  commit `72b73fc`; v1.4.1 commit on `v1.1-fixes`; v1.5.0 commit
  `e57e4fd`; v1.5.1 commit on `pr-9-audit-hardening`; v1.5.2 commit
  `30ff667`; v1.6.0 commit `d551320` (PR #23, frontier hardening +
  governed signal layer + mypy 13->0); v1.6.1 commit on
  `pr-24-v1.6.1-version-identity` (release-identity sync over v1.6.0).
<!-- ci-status-end -->
