...
<!-- ci-status-start -->
- **Tests:** 1006 passing (`pytest -q -m "not slow"`) on the v1.5.1
  PR-9 branch; v1.5.0 baseline was 916.
- **Ruff:** `ruff check src tests` and `ruff format --check src tests`
  exit 0.
- **Mypy:** 14 errors in 8 files (v1.5.0 baseline 34; PR-9 FIX 7
  brought down the top offenders in `bocpd_muse.py`, `wfst.py`,
  `cli_dispatch.py`, and a long tail of unused `# type: ignore`
  comments).
...
- Initial commit `741e51a`; v1.1 commit `79249df`; v1.2 commit
  ...
  `e57e4fd`; v1.5.1 commit on `pr-9-audit-hardening`.
<!-- ci-status-end -->
