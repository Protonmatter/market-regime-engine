...
v1.6 production posture:

- Non-production defaults to ``data/mre.duckdb`` so API and CLI share the
  DuckDB-primary warehouse path.
...
class _RedisTTLCache:
    ...
    def set(self, key: str, value: object) -> None:
        try:
            self.client.set(self._scoped(key), pickle.dumps(value), ex=int(_ttl_seconds()))
        except Exception as exc:  # pragma: no cover - transport failure
...
def v1_health() -> dict:
    ...
    try:
        ...
        db.close()
    return {"status": "ok", "version": __version__, "release_gate": latest_decision, "production": _PRODUCTION_CHECK.production}
...
def v1_analogs_latest() -> dict:
    def _fn(db: Warehouse) -> dict:
        ...
        return {"as_of_date": latest_date, "summary": analog_summary(latest), "analogs": latest.to_dict(orient="records")}

    return _read("analogs_latest", _fn)  # type: ignore[return-value]


__all__ = ["_LocalTTLCache", "_RedisTTLCache", "_TTLCache", "_build_cache_backend", "app", "require_api_key"]
