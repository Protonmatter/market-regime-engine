...
def normalize_governed_signals(frame: pd.DataFrame, *, default_release_gate_decision: str = "unknown") -> pd.DataFrame:
    ...
    out["regime_state"] = _first_existing(src, ("regime_state", "decoded_regime", "regime", "msvar_regime", "state"), "unknown").astype(str)
    out["regime_confidence"] = pd.to_numeric(_first_existing(src, ("regime_confidence", "msvar_confidence", "score", "confidence"), 0.0), errors="coerce").fillna(0.0)
    out["change_point_prob"] = pd.to_numeric(_first_existing(src, ("change_point_prob", "cp_prob", "bocpd_prob"), 0.0), errors="coerce").fillna(0.0)
    out["drawdown_prob"] = pd.to_numeric(_first_existing(src, ("drawdown_prob", "p_drawdown", "drawdown_probability"), 0.0), errors="coerce").fillna(0.0)
    out["recession_prob"] = pd.to_numeric(_first_existing(src, ("recession_prob", "p_recession", "recession_probability"), 0.0), errors="coerce").fillna(0.0)
    out["confidence_score"] = pd.to_numeric(_first_existing(src, ("confidence_score", "confidence"), out["regime_confidence"]), errors="coerce").fillna(out["regime_confidence"])
    out["release_gate_decision"] = _first_existing(src, ("release_gate_decision", "decision"), default_release_gate_decision).astype(str)
    ...
    if "metadata_json" in src:
        ...
        out["metadata_json"] = [json.dumps({"adapter_contract": "governed_macro_regime_signal_v1"}, sort_keys=True) for _ in range(len(out))]
...
def assert_governed_signal_contract(frame: pd.DataFrame) -> None:
    ...
    for col in ("regime_confidence", "change_point_prob", "drawdown_prob", "recession_prob", "confidence_score"):
...
def export_governed_signals(frame: pd.DataFrame, out_path: str | Path, *, fmt: Literal["csv", "json", "jsonl"] | None = None) -> GovernedSignalExport:
    out = Path(out_path)
    ...
    return GovernedSignalExport(path=str(out), rows=int(len(signals)), format=fmt, columns=tuple(signals.columns))


__all__ = ["GOVERNED_SIGNAL_COLUMNS", "GovernedSignalExport", "assert_governed_signal_contract", "export_governed_signals", "normalize_governed_signals", "parse_bool_series"]
