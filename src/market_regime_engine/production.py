...
The engine is allowed to be friendly in local research mode. In production it
must be boring, explicit, and hostile to ambiguity. This module centralises the
runtime checks so API, orchestration, and future CLIs can share the same
fail-closed posture instead of copy/pasting env-var folklore.
"""
...
def select_api_db_path(env: Mapping[str, str] | None = None, *, default: str = "data/mre.duckdb") -> str:
    ...
    Non-production keeps a safe DuckDB-first default. Production requires an
    explicit ``MRE_DB_PATH`` so an API server cannot accidentally point at a
    local smoke-test SQLite file.
    """
