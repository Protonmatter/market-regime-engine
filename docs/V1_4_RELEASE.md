...
|---|------|---------------------------|-----------------|--------------------|
| A | Bayesian NumPyro MS-VAR | `src/market_regime_engine/frontier/bayesian_msvar.py:1-527` (BayesianMSVAR(p) + Dirichlet/LKJ priors + ordered-state anchor + companion-radius diagnostics); `src/market_regime_engine/storage.py:427-456` (bayesian_msvar_diagnostics table); `src/market_regime_engine/cli.py` (bayesian-msvar-fit subcommand) | `tests/test_bayesian_msvar.py` (5 tests: NUTS converges, SVI fallback, EM-parity within L1<0.30, soft-degrade, BMA plug-in) | EM MS-VAR collapsed every regime indicator to the posterior mean with no credible bands; downstream BMA could not refuse to act on an over-confident point estimate. Bayesian gives posterior uncertainty + R-hat / ESS diagnostics and now supports a true AR(p) lag stack. |
